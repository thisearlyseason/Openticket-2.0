...
import { Calendar, MapPin, Clock, Share2, Ticket, Check, AlertCircle, Info, Lock, Users, Printer, FileText, Download, Gift, Hourglass, CheckCircle, ArrowRight, Target, Image as ImageIcon, QrCode, Heart, Loader, Eye, X, Key } from 'lucide-react';
...
export const EventView = () => {
                            ...
                            if (verifyResponse.ok) {
                                ...
                                if (result.status === 'success' && result.registration) {
                                    ...
                                    const normalizedReg: any = {
                                        ...
                                        customFeesAmount: reg.custom_fees_amount || reg.answers?._metadata?.custom_fees_amount || 0,
                                        taxAmount: reg.tax_amount || 0,
                                        stripeFee: reg.stripe_fee || reg.answers?._metadata?.stripe_fee || 0,
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            if (paymentMethod === 'online' && finalTotal > 0) {
                ...
                const response = await fetch('/api/stripe/create-order', {
                    ...
                    body: JSON.stringify({
                        ...
                        successUrl: `${window.location.origin}/?stripe_return=true&event_id=${event.id}&success=true`,
                        cancelUrl: `${window.location.origin}/?stripe_return=true&event_id=${event.id}&canceled=true`,
            ...
            const newReg: Registration = {
                ...
                discountAmount: orderBreakdown?.discountAmount || 0,
    ...
    return (
        <>
                            ...
                            <div className="absolute bottom-0 left-0 w-full p-6 md:p-12">
                                <div className="max-w-7xl mx-auto">
                                    ...
                                    <div className="flex flex-wrap gap-6 items-center text-white/90 font-bold bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/10 w-fit animate-in fade-in zoom-in-95 duration-1000">
                                        <div className="flex items-center gap-3"><Calendar size={28} className="text-pink-500" /> {new Date(event.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
                                        <div className="w-px h-8 bg-white/20 hidden md:block"></div>
                                        <div className="flex items-center gap-3"><Clock size={28} className="text-pink-500" /> {formatTime(event.time, event.timeFormat)}</div>
                                        <div className="w-px h-8 bg-white/20 hidden md:block"></div>
                                            ...
                                            <div className="space-y-6">
                                                    ...
                                                    <>
                                                        <div className={`p-8 border-2 transition-all rounded-[2rem] flex flex-col md:flex-row justify-between items-center ${ticketSelection['general'] ? 'border-primary bg-primary/5 shadow-2xl shadow-primary/10' : 'border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-black/20'}`}>
                                                            <div className="text-center md:text-left mb-6 md:mb-0">
                                                                <div className="font-black text-2xl mb-1 uppercase tracking-tighter text-zinc-900 dark:text-white">{event.ticketName || 'General Admission'}</div>
                                                            ...
                                                            <div className="flex items-center gap-6 bg-white dark:bg-zinc-800 p-3 rounded-[2rem] shadow-xl">
                                                                <button onClick={() => handleTicketChange('general', Math.max(0, (ticketSelection['general'] || 0) - 1))} className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-zinc-700 text-zinc-900 dark:text-white flex items-center justify-center hover:bg-zinc-200 transition-colors font-bold text-2xl disabled:opacity-30" disabled={!ticketSelection['general']}>-</button>
                                                                <span className="w-10 text-center font-black text-3xl text-zinc-900 dark:text-white">{ticketSelection['general'] || 0}</span>
                                                ...
                                                {event.addOns && event.addOns.length > 0 && (
                                                    <div className="mt-12 pt-12 border-t border-zinc-100 dark:border-zinc-800">
                                                        <h3 className="text-2xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3"><Gift size={32} className="text-pink-500" /> Extra Goodies</h3>
                                                        <div className="space-y-6">
                                                            {event.addOns.map(addon => (
                                                                <div key={addon.id} className="p-8 bg-zinc-50 dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-8 group hover:border-pink-500/30 transition-all">
                                                                    <div className="flex-1 text-center md:text-left">
                                                                        <div className="font-black text-xl flex flex-col md:flex-row items-center gap-3">
                                                                            {addon.name}
                                                                            <span className="text-zinc-900 dark:text-white font-black bg-zinc-200 dark:bg-zinc-800 px-3 py-1 rounded-full text-lg"><EventPriceDisplay amount={addon.price} currency={eventCurrency} /></span>
                                                                        </div>
                                                                    </div>
                                                                    <div className="flex items-center gap-4 bg-white dark:bg-zinc-800 p-2 rounded-[1.5rem] shadow-lg">
                                                                        ...
                                                                        <button onClick={() => setAddOnSelection(prev => ({ ...prev, [addon.id]: { ...(prev[addon.id] || { qty: 0 }), qty: (prev[addon.id]?.qty || 0) + 1 } }))} className="w-10 h-10 rounded-xl bg-secondary text-black flex items-center justify-center hover:scale-110 active:scale-90 transition-all font-bold text-xl">+</button>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                                ...
                                                <div className="mt-12 p-4 sm:p-8 bg-zinc-100 dark:bg-zinc-800/50 rounded-2xl sm:rounded-[2.5rem] border-2 border-dashed border-zinc-300 dark:border-zinc-700 group hover:border-primary/50 transition-colors">
                                                            ...
                                                            <div className="space-y-2 text-sm font-medium">
                                                                ...
                                                                {event.priceType === 'tiered' ? (
                                                                    event.ticketTiers?.map(tier => {
                                                                        ...
                                                                        return (
                                                                            <div key={tier.id} className="flex justify-between text-zinc-600 dark:text-zinc-400">
                                                                                ...
                                                                                <span>{tier.price === 0 ? 'FREE' : <EventPriceDisplay amount={tier.price * qty} currency={eventCurrency} />}</span>
                                                                            </div>
                                                                        );
                                                                    })
                                                                ) : (
                                                                    (ticketSelection['general'] || 0) > 0 && (
                                                                        <div className="flex justify-between text-zinc-600 dark:text-zinc-400">
                                                                            ...
                                                                            <span>{event.priceType === 'free' ? 'FREE' : event.priceType === 'donation' ? <EventPriceDisplay amount={Number(regData.donation) || 0} currency={eventCurrency} /> : <EventPriceDisplay amount={event.price * (ticketSelection['general'] || 0)} currency={eventCurrency} />}</span>
                                                                        </div>
                                                                    )
                                                                )}
                                                                ...
                                                                {event.addOns?.map(addon => {
                                                                    ...
                                                                    return (
                                                                        <div key={addon.id} className="flex justify-between text-zinc-600 dark:text-zinc-400">
                                                                            ...
                                                                            <span><EventPriceDisplay amount={addon.price * qty} currency={eventCurrency} /></span>
                                                                        </div>
                                                                    )
                                                                })}
                                                                ...
                                                                {regData.platformDonationAmount > 0 && (
                                                                    <div className="flex justify-between text-pink-600 dark:text-pink-400">
                                                                        ...
                                                                        <span><EventPriceDisplay amount={regData.platformDonationAmount} currency={eventCurrency} /></span>
                                                                    </div>
                                                                )}
                                                            </div>
                    ...
                    <div className={isWidget ? 'hidden' : 'lg:col-span-4 space-y-8'}>
                        ...
                        <Card className="p-8 bg-zinc-50 dark:bg-zinc-900/50 border-zinc-200 dark:border-zinc-800 rounded-[3rem] text-center">
                                    ...
                                    onClick={async () => {
                                        try {
                                            ...
                                            if (e.name !== 'AbortError') {
                                                showToast("Couldn't open share menu", "error");
                                            }
                                        }
                                    }}
                                    className="w-full mb-4 md:hidden"
                                >
                                    <Share2 size={18} className="mr-2" /> Share via Apps
                                </Button>
                            )}
                            ...
                            <div className="grid grid-cols-5 gap-3 mb-6">
                                <button 
                                    onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out ${event.title}!`)}&url=${encodeURIComponent(shareUrl)}`, '_blank', 'noopener,noreferrer')}
                                ...
                                <button 
                                    onClick={async () => {
                                        // Instagram doesn't support direct sharing - copy and guide user
                                        const shareText = `${event.title} 🎉\n\n${metaDesc}\n\n${shareUrl}`;
                                        try {
                                            await navigator.clipboard.writeText(shareText);
                                            showToast("Link copied! Instagram doesn't support direct sharing. Paste in your Story or Bio.", "success");
                                            window.open('https://www.instagram.com/', '_blank', 'noopener,noreferrer');
                                        } catch (e) {
                                            showToast("Please copy the link manually and paste it on Instagram.", "info");
                                        }
                                    }}
                                    className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl hover:bg-gradient-to-tr hover:from-[#f9ce34]/20 hover:via-[#ee2a7b]/20 hover:to-[#6228d7]/20 transition-colors group"
                                    title="Share on Instagram (copies link)"
                                >
                                    <svg className="w-6 h-6 mx-auto text-zinc-600 dark:text-zinc-400 group-hover:text-pink-600" viewBox="0 0 24 24" fill="currentColor">
                                ...
                                <button 
                                    onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`Check out ${event.title}! ${shareUrl}`)}`, '_blank', 'noopener,noreferrer')}
            ...
            {/* Post-Stripe Payment Verification Overlay - Shows ONLY after returning from Stripe */}
            {
                isProcessingPayment && (
                    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-6 animate-in fade-in duration-500">
                        <div className="max-w-md w-full bg-zinc-900 border border-white/10 rounded-[3rem] p-12 text-center shadow-[0_0_100px_rgba(34,197,94,0.15)] relative overflow-hidden">
                            ...
                            <div className="relative">
                                ...
                                <p className="text-zinc-400 font-bold mb-8">
                                    Almost there! Confirming your purchase with Stripe...
                                </p>

                                <div className="flex items-center justify-center gap-6 px-4 py-3 bg-green-500/10 rounded-2xl border border-green-500/20">
                                    <span className="text-xs font-black uppercase tracking-widest text-green-500">Processing</span>
                                    <div className="h-1 w-1 bg-green-500 rounded-full animate-pulse"></div>
                                    <span className="text-xs font-black uppercase tracking-widest text-zinc-500">Please Wait...</span>
                                </div>
                            </div>
                        </div>
                    </div>
