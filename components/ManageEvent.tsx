...
import { Button, Input, Card, Badge, RichTextarea } from './UI';
import { 
    Edit, QrCode, Users, Calendar, MapPin, 
    ArrowLeft, Eye, Mail, Send, Sparkles, 
    AlertTriangle, Check, List, BarChart3, Megaphone, 
    Wallet, Settings, DollarSign, ExternalLink
} from 'lucide-react';
...
export const ManageEvent = () => {
    ...
    const handleSendBroadcast = async () => {
        ...
        try {
            const count = await StorageService.sendEventBroadcast(event.id, broadcastSubject, broadcastBody);
            alert(`Sent to ${count} attendees!`);
            setBroadcastSubject('');
            setBroadcastBody('');
        ...
        <div className="min-h-screen pb-24">
            ...
            <div className="relative h-64 md:h-80 w-full overflow-hidden">
                <div className="absolute inset-0 bg-zinc-900">
                    {event.imageUrl && <img src={event.imageUrl} className="w-full h-full object-cover opacity-60 blur-sm" />}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-6">
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="text-zinc-400 text-sm font-bold flex items-center gap-1"><Calendar size={14}/> {new Date(event.date).toLocaleDateString()}</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter drop-shadow-lg">{event.title}</h1>
                        <div className="flex items-center gap-2 text-zinc-300 text-sm mt-1 font-medium">
                    ...
                    <div className="space-y-6">
                        <Card className="p-6 border-zinc-200 dark:border-zinc-800">
                            ...
                            <div className="space-y-2">
                                <button onClick={() => navigate(`/edit/${event.id}`)} className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors text-sm">
                                    <span className="flex items-center gap-2"><Settings size={16}/> General Settings</span>
