...
export const AttendeeManager = () => {
    ...
    const handleApproveAttendee = async (item: AttendeeItem) => {
        ...
        try {
            ...
            if (event.ownerId && item.email) {
                const eventDate = new Date(event.date).toLocaleDateString('en-US', { 
                    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
                });
                const eventLocation = event.location || event.venueName || 'TBD';
                
                const subject = `🎉 Approved! Your Registration for ${event.title}`;
                const body = `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f5f5f5; padding: 40px 20px;">
        <tr>
            <td align="center">
                <table width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    ...
                    <tr>
                        <td style="padding: 40px 30px;">
                            ...
                            <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 0 0 30px 0;">
                                Great news! Your registration for <strong>${event.title}</strong> has been approved by the organizer.
                            </p>
                            <table width="100%" style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; margin-bottom: 30px;">
                                <tr>
                                    <td style="padding: 20px;">
                                        <p style="color: #111827; font-size: 18px; font-weight: 600; margin: 0 0 10px 0;">${event.title}</p>
                    ...
                    <tr>
                        <td style="background-color: #f9fafb; padding: 30px; text-align: center; border-top: 1px solid #e5e7eb;">
                            <p style="color: #9ca3af; font-size: 12px; margin: 0;">Organized by ${event.organizer || 'Event Organizer'}</p>
                        </td>
                    </tr>
                </table>
            </td>
                ...
                EmailService.sendEmail(event.ownerId, item.email, subject, body).catch(console.error);
            }
            await loadData();
    ...
    const handleResendEmail = async (item: AttendeeItem) => {
        ...
        showConfirm({
            ...
            onConfirm: async () => {
                try {
                    // Use backend API for sending emails (backend-only email policy)
                    const result = await StorageService.resendConfirmationEmail(item.regId);
