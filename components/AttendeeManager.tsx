...
export const AttendeeManager = () => {
    ...
    const handleApproveAttendee = async (item: AttendeeItem) => {
        if (!event) return;
        try {
            // Use backend API for approval (includes email sending)
            const result = await StorageService.approveRegistration(item.regId);
            
            if (result.error) {
                throw new Error(result.error);
            }
            
            showToast(result.message || "Registration approved!", "success");
            await loadData();
        } catch (e: any) { 
