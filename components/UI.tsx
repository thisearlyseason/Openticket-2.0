...
export const ReceiptModal = ({ isOpen, onClose, registration, event, organizer }: { isOpen: boolean, onClose: () => void, registration: Registration, event: Event, organizer?: User }) => {
    ...
    const platformFee = convert(usdPlatformFee);
    const stripeFee = convert(usdStripeFee);
    const customFees = convert(usdCustomFees);
                        ...
                        <div className="border-t border-gray-200 pt-4 space-y-2 text-sm">
                            ...
                            {stripeFee > 0 && (
                                <div className="flex justify-between text-gray-600">
                                    ...
                                    <span>{formatAmount(stripeFee)}</span>
                                </div>
                            )}
                            {customFees > 0 && (
                                <div className="flex justify-between text-gray-600">
                                    <span>Additional Fees</span>
                                    <span>{formatAmount(customFees)}</span>
                                </div>
                            )}
                            {tax > 0 && (
                                <div className="flex justify-between text-gray-600">
