                        ...
                        {isEditingStripe || !user.stripeConnectId ? (
                            <div className="bg-white/10 p-6 rounded-xl backdrop-blur-sm animate-in fade-in space-y-4">
                                <div className="text-center">
                                    ...
                                    <Button
                                        onClick={async () => {
                                            const originalText = document.getElementById('connect-btn')?.innerText;
                                            const btn = document.getElementById('connect-btn');
                                            if (btn) btn.innerText = "Connecting...";

                                            try {
                                                const res = await StorageService.connectStripeAccount(user.id, 'standard');
                                                if (res.success) {
                                                    alert("Stripe Connected Successfully!");
                                                    window.location.reload();
                                                }
                                                ...
                                                if (btn) btn.innerText = originalText || "Connect with Stripe";
                                            }
                                        }}
                                        id="connect-btn"
