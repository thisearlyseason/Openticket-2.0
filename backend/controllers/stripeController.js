...
export const createOrder = async (req, res) => {
    try {
        ...
        let requestedQty = 0;
        Object.values(ticketSelections).forEach((val: any) => requestedQty += (val.qty || 0));
        ...
        for (const [ticketId, selection] of Object.entries(ticketSelections)) {
            ...
            if (selection.qty > 0) {
                ...
                if (tier) {
                    ...
                    for (let i = 0; i < qty; i++) {
                        ticketsData.push({
                            id: `tix-${Date.now()}-${i}`, // Temporary ID
                            tierId: ticketId,
                            name: tier.name,
        ...
        if (assignments) {
            let ticketIndex = 0;
            // Iterate ticketsData and assign names
            // This is complex, simplified for now: matches order of creation
            /* 
            assignments: { 'ticketId': [{name, email}, ...] } 
            */
